...
export class Grid {
    ...
    public mapSize: { xMin: number; yMin: number; xMax: number; yMax: number };
    public gridPos(coord: Coord): GridPosition {
        return this.grid[coord.x][coord.y];
    }
    public constructor(public gridSize: Coord, public BASE_COST: number) {
        ...
        this.mapSize = { xMin: 0, xMax: gridSize.x, yMin: 0, yMax: gridSize.y };
    }
    /**
     * 获取两个坐标之间的距离
     *
    ...
    private ifInBorder(coord: Coord): boolean {
        const { x, y } = coord;
        const { xMin, yMin, xMax, yMax } = this.mapSize;
